...
Welcome! This tutorial is designed for complete beginners. If you have never built a Oculus0Osint plugin before, you are in the right place. 
...
Before we write any code, we need to prepare your computer. We will install the required tools and download the two main repositories that make up the Oculus0Osint platform.
...
You only need the main Oculus0Osint application. The data engine backend will run automatically via Docker. Open your terminal and navigate to the folder where you want to store your code (e.g., `C:\dev`).
...
Next, install dependencies for the main Oculus0Osint application:
```bash
cd Oculus0Osint
pnpm install
```
...
Oculus0Osint needs a few basic configuration keys to run. 

While still in the `Oculus0Osint` folder, run the automated setup script. This will generate a `.env.local` file with the necessary secrets:
```bash
pnpm run setup
```
...
Navigate to your `Oculus0Osint` repository. Inside the `local-seeders` folder, create a new folder named `community/iss`. Inside that folder, create a file named `seeder.mjs`:
`c:\dev\Oculus0Osint\local-seeders\community\iss\seeder.mjs`
...
In your terminal, navigate to the `Oculus0Osint` folder and start the entire stack using Docker Compose:
```bash
pnpm dev:all
```
...
## Step 4: Creating the Oculus0Osint Plugin (Frontend)
...
In your terminal, navigate to your main `Oculus0Osint` folder. We will use the workspace CLI tool to generate a blank plugin template in the local sandboxes folder. Run:
...
Open `c:\dev\Oculus0Osint\local-plugins\wwv-plugin-iss\src\index.ts` in your text editor. Replace everything in the file with this code:
...
Now we will link your new plugin to your local Oculus0Osint application to see it in action.
...
Open `c:\dev\Oculus0Osint\local-plugins\wwv-plugin-iss\package.json`. You must add a `"worldwideview"` metadata block so the marketplace knows how to read your plugin. Ensure your file looks like this:
...
3. The system will automatically verify your code. Once approved, any Oculus0Osint user on the planet can click "Install" to add your ISS tracker to their globe.
