...
## Overview
Oculus0Osint operates on a decoupled, plugin-based architecture designed for real-time visualization of geospatial data. The system is split between a Next.js 16 frontend that handles UI and CesiumJS 3D rendering, and a network of plugin data seeders that stream live updates via WebSockets.
...
1. **Engine Push:** A remote data engine pushes state over WebSocket (`/stream`) from a local seeder process (default `ws://localhost:5000/stream`). The engine URL is resolvable through `src/core/data/resolveEngineUrl.ts`; in a self-hosted local edition the local seeder is the only path that fires.
