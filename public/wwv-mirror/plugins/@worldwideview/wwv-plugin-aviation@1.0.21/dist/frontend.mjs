...
var R = class extends I {
	constructor(...e) {
		super(...e), this.id = "aviation", this.name = "Aviation", this.description = "Real-time aircraft tracking via OpenSky Network", this.icon = F, this.version = "1.0.8";
	}
	getAltitudeColor(e) {
	...
	async fetch(e) {
		try {
			let e = this.context.getEngineUrl(), t;
			if (this.context.isPlaybackMode()) {
				let n = this.context.getCurrentTime().getTime();
				t = await fetch(`${e}/api/aviation?time=${n}`);
			} else t = await fetch(`${e}/api/aviation?lookback=15m`);
