...
function parseAircraftState(state: OpenSkyState) {
    const callsign = typeof state[1] === "string" ? state[1].trim() || null : null;
    return { icao24: state[0], callsign, origin_country: state[2],
...
export async function GET() {
    try {
        ...
        const states = Array.isArray(data?.states) ? data.states as OpenSkyState[] : [];
        const features = states.filter((s: OpenSkyState) => s[5] !== null && s[6] !== null).map((s: OpenSkyState) => {
